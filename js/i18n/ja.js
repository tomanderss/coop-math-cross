...
export default {
  ...
  tutor: {
    ...
    next: '次へ',
    apply: 'わかった – 実行',
    direct: { look: 'この式を見てください：{eq}。ここに足りない数は 1 つだけです。', solve: 'だから一意に計算できます：{eq}。' },
    combo: { look: 'この式を見てください：{eq}。数が複数足りません…', only: '…でも手持ちから入る組み合わせは 1 通りだけです（{n} マス）。' },
    cross: { look: 'このマスは同時に {n} 個の式に属しています。', only: '両方に入る数は 1 つだけです：{a} と {b}。' },
    pool: { look: '{v} は手持ちに {n} 個あります。', only: 'そして入るマスはちょうど {n} 個 – どれにも {v} が入ります。' },
    place: 'だからここには {v} が入ります。',
    placeMulti: 'これで {n} マスが決まりました。',
  },
  achievements: {
    title: '実績',
  ...
  howto: {
    ...
    rule3: '使うのは<b>正の整数だけ</b> — 小数もマイナスの答えもありません。',
    rule4: '埋まっているマスに置くと 2 つのタイルが<b>入れ替わり</b>ます。<b>並べ替えボタン</b>で手持ちを昇順に整えられます。',
